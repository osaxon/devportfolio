...
export const FEATURED_PROJECTS = gql`
...
query FeaturedPosts {
  posts(first: 6, orderBy: publishedAt_DESC) {
    id
    slug
    title
    excerpt
...
function withImageUrl(data){
  return data.map((d) => {
    return {...d, coverImage: buildImage(d.coverImage.public_id).resize('w_800,h_600').toURL()}
  })
}
...
export async function getFeaturedPosts() {
  ...
  return posts.map((p) => {
    return {...p, coverImage: buildImage(p.coverImage.public_id).resize('w_800,h_600').toURL()}
  });
}
